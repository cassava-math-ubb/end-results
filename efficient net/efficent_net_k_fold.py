...
WORK_DIR = '/bigdata/users-data/aaic2261/cassava/'
...
if __name__ == '__main__':
    ...
    for train_index, val_index in skf.split(np.zeros(n), Y):
        ...
        model_save = ModelCheckpoint(f"k_fold_saved_models/best_model_{str(fold_var)}.h5",
                                     save_best_only=True,
        ...
        reduce_lr = ReduceLROnPlateau(monitor='val_loss',
                                      ...
                                      min_delta=0.001,
                                      mode='min')
        tensorboard = TensorBoard(log_dir=f"k_fold_logs/scalars/{datetime.now().strftime('%Y%m%d-%H%M%S')}")
        ...
        with open(f"k_fold_saved_models/model_{str(fold_var)}.json", "w") as json_file:
            json_file.write(model_json)

        # evaluate validation dataset using the best weights
        model.load_weights(f"k_fold_saved_models/best_model_{str(fold_var)}.h5")
